...
export function CategoryControls({ showHero, plantId, flowId, plants, flows }: Props) {
  ...
  const buildUrl = (partial: Record<string, string | undefined>) => {
    ...
    if (partial.showHero !== undefined) {
      if (partial.showHero) params.set("showHero", partial.showHero);
  ...
  return (
    <div className="flex flex-wrap items-center gap-3">
      <Link
        ...
        className="rounded-md border border-smc-border/70 bg-white px-3 py-2 text-sm font-semibold text-smc-text hover:bg-smc-bg/60"
      >
        Back
      </Link>
      <button
      ...
      <form className="flex flex-wrap items-center gap-2" onSubmit={(e) => e.preventDefault()}>
        ...
        <label className="text-sm text-smc-text">Flow</label>
        <select
          name="flowId"
          value={flowId}
