...
describe("AutoTranstockerForm edit mode", () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });
  ...
  it("prefills existing values", () => {
    ...
    expect(screen.getByDisplayValue(/Existing auto transtocker/i)).toBeInTheDocument();
    ...
    expect(screen.getByPlaceholderText(/e\.g\. siemens/i)).toHaveValue("Siemens");
  ...
  it("submits updates and redirects on success", async () => {
    ...
    fireEvent.change(screen.getByPlaceholderText(/e\.g\. siemens/i), { target: { value: "Rockwell" } });
    fireEvent.click(screen.getByRole("button", { name: /pick plant/i }));
    fireEvent.click(screen.getByRole("button", { name: /pick flow/i }));
    ...
    expect(formData.get("plcType")).toBe("Rockwell");
    ...
    await waitFor(() => expect(pushMock).toHaveBeenCalledWith(`/storage-means/auto-transtocker/${storageMean.id}`));
  ...
  it("does not submit when adding inline plant/flow/supplier and only submits on final action", async () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: /start/i }));
    await screen.findByDisplayValue("Existing auto transtocker");
