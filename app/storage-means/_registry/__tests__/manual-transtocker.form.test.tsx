...
const fillBasics = () => {
  ...
  fireEvent.change(screen.getByPlaceholderText(/storage mean name/i), { target: { value: "Manual transtocker" } });
  ...
  fireEvent.change(screen.getByPlaceholderText(/what makes this storage mean unique/i), {
    target: { value: "A robust manual transtocker" },
  });
...
const attachImage = (fileName = "manual.png") => {
...
describe("ManualTranstockerForm", () => {
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  it("submits successfully and redirects", async () => {
    ...
    expect(formData.get("name")).toBe("Manual transtocker");
    ...
    await waitFor(() => expect(pushMock).toHaveBeenCalledWith("/storage-means/manual-transtocker"));
