...
describe("storage mean actions", () => {
  beforeEach(() => {
    ...
    (mockedPrisma.laneGroup.create as jest.Mock).mockResolvedValue({ id: "lane-group-1" });
    (mockedPrisma.lane.create as jest.Mock).mockResolvedValue({ id: "lane-1" });
  ...
  const buildLanesJson = (lanes = [{ lengthMm: 100, widthMm: 50, heightMm: 60, numberOfLanes: 2 }]) => JSON.stringify(lanes);
  ...
  it("creates a manual storage mean with lanes and images", async () => {
    ...
    expect(mockedPrisma.storageMeanFlow.createMany).toHaveBeenCalledWith(
      expect.objectContaining({ data: expect.arrayContaining([expect.objectContaining({ flowId: "33333333-3333-4333-8333-333333333333" })]) })
    );
    expect(mockedPrisma.laneGroup.deleteMany).toHaveBeenCalledWith({ where: { storageMeanId: storageMeanId } });
    expect(mockedPrisma.laneGroup.create).toHaveBeenCalledWith(
      expect.objectContaining({ data: expect.objectContaining({ storageMeanId: storageMeanId }) })
    );
    expect(mockedPrisma.lane.create).toHaveBeenCalledWith(
      expect.objectContaining({ data: expect.objectContaining({ laneGroupId: "lane-group-1" }) })
    );
  ...
  it("updates an auto storage mean, replacing images and lanes", async () => {
    ...
    fd.append("flowIds", JSON.stringify(["33333333-3333-4333-8333-333333333333"]));
    fd.append("supplierId", "44444444-4444-4444-8444-444444444444");
    fd.append("lanes", buildLanesJson([{ lengthMm: 200, widthMm: 80, heightMm: 70, numberOfLanes: 4 }]));
    ...
    expect(mockedPrisma.storageMeanFlow.deleteMany).toHaveBeenCalledWith({ where: { storageMeanId: storageMeanId } });
    expect(mockedPrisma.storageMeanFlow.createMany).toHaveBeenCalledWith(
      expect.objectContaining({ data: expect.arrayContaining([expect.objectContaining({ flowId: "33333333-3333-4333-8333-333333333333" })]) })
    );
    expect(mockedPrisma.laneGroup.deleteMany).toHaveBeenCalledWith({ where: { storageMeanId: storageMeanId } });
