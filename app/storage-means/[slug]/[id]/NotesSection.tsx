...
import { createStorageMeanNoteAction, type NoteActionState } from "../../actions";
...
export default function NotesSection({ storageMeanId, slug, initialNotes }: NotesSectionProps) {
      ...
      {showForm ? (
        <form
          ...
          onSubmit={handleSubmit((values) => {
            ...
            fd.append("storageMeanId", storageMeanId);
            fd.append("slug", slug);
            fd.append("title", values.title);
            fd.append("content", values.content);
            startTransition(async () => {
              const result = await createStorageMeanNoteAction(state, fd);
              setState(result);
          ...
          className="space-y-3 rounded-lg border border-dashed border-smc-border/60 bg-smc-bg/50 p-3"
        >
          <div className="space-y-1">
            <label className="text-xs font-semibold text-smc-text" htmlFor="storage-note-title">
              Title <span className="text-red-600">*</span>
            </label>
            <input
              id="storage-note-title"
              type="text"
              {...register("title")}
          ...
          <div className="space-y-1">
            <label className="text-xs font-semibold text-smc-text" htmlFor="storage-note-content">
              Note <span className="text-red-600">*</span>
            </label>
            <textarea
              id="storage-note-content"
              {...register("content")}
              className="w-full rounded-md border border-smc-border/70 px-3 py-2 text-sm"
              rows={3}
