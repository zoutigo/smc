...
export default function StorageForm({
  ...
  return (
    <form onSubmit={onFormSubmit} className="space-y-4 rounded bg-white p-4 shadow">
      <div>
        <label htmlFor="storage-name" className="block text-sm font-medium">Storage category name</label>
        <input id="storage-name" className="mt-1 w-full" {...register("name")} />
        {errors.name && <p className="text-sm text-red-600">{errors.name.message}</p>}
      </div>

      <div>
        <label htmlFor="storage-description" className="block text-sm font-medium">Description</label>
        <textarea id="storage-description" className="mt-1 w-full resize-none" rows={4} {...register("description")} />
        {errors.description && <p className="text-sm text-red-600">{errors.description.message}</p>}
      </div>

      <div>
        <label htmlFor="storage-image" className="block text-sm font-medium">Image upload</label>
        <input
          id="storage-image"
          type="file"
          accept="image/*"
        ...
        {showPreviewMeta && (
          <div className="mt-3 flex flex-wrap items-center gap-3 rounded-2xl border border-dashed border-slate-200 bg-slate-50 p-3">
            {previewImage ? (
              ...
              <div className="flex h-14 w-14 items-center justify-center rounded-full border border-slate-300 bg-white text-xs text-slate-400" data-testid="storage-image-placeholder">
                No img
              </div>
            )}
