...
type FormComponentProps = {
  onClose: () => void;
  onSuccess: () => void;
  initialValues?: { id?: string; name?: string; description?: string; imageUrl?: string };
  mode?: "create" | "edit";
  submitLabel?: string;
  successMessage?: string;
};

type Props<T extends CategoryLike> = {
  categories: T[];
  baseHref: string; // e.g., "/packaging-means" or "/means/packaging"
  label: string; // uppercase label shown on cards
  heading: string;
  subheading: string;
  ...
  footerTestId?: string;
  labelTestId?: string;
  deleteTitle: (c: T) => string;
  deleteDescription: (c: T) => string;
  FormComponent: React.ComponentType<FormComponentProps>;
  onDelete: (id: string) => Promise<void>;
};

export function CategoryCataloguePageClient<T extends CategoryLike>({
  categories,
  baseHref,
  label,
  heading,
      ...
      <section className="overflow-hidden rounded-[28px] border border-smc-border/60 bg-gradient-to-r from-white via-smc-bg/60 to-white p-7 shadow-card">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="space-y-2">
            <p className="text-xs font-semibold uppercase tracking-[0.35em] text-smc-primary/80">{label}</p>
            <div className="flex flex-wrap items-end gap-3">
              <h1 className="heading-1 leading-[1.05]">{heading}</h1>
              <div className="rounded-full bg-smc-primary/10 px-3 py-1 text-xs font-semibold text-smc-primary" data-testid="category-count-heading">
                {totalItems} total
              </div>
            </div>
            <p className="max-w-2xl text-sm text-smc-text-muted">{subheading}</p>
          </div>
          <div className="flex flex-col items-start gap-2 sm:items-end">
            <CustomButton onClick={handleToggle} className="self-stretch sm:self-auto" text={showForm ? "Hide form" : addButtonText} />
      ...
      <div className="rounded-[26px] border border-smc-border/80 bg-white/85 p-6 shadow-card ring-1 ring-white/70 backdrop-blur">
        <div className={layoutClassName}>
          <div className="space-y-6">
            <div data-testid="category-cards-grid" className={cardsGridClassName}>
              {paginatedCategories.map((category) => (
                <CategoryCard
                  ...
                  editAriaLabel={`Edit ${label.toLowerCase()} category`}
                  deleteAriaLabel={`Delete ${label.toLowerCase()} category`}
                  deleteTitle={deleteTitle(category)}
                  deleteDescription={deleteDescription(category)}
            ...
            <div className="flex justify-center" data-testid="category-pagination">
              <Pagination totalItems={totalItems} currentPage={currentPage} pageSize={pageSize} onPageChange={setPage} />
            </div>
          </div>
